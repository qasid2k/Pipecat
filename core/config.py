...
def _load_persona(data: dict, base_dir: Path) -> PersonaConfig:
    ...
    prompt = prompt.replace("{name}", name).replace("{company}", company)
    greeting = d.get(
...
def load_config(path: str | Path | None = None) -> AppConfig:
    ...
    top = _section(raw, "(top level)", allowed={"transport", "engine"}, required={"transport", "engine"})
    ...
    return AppConfig(transport=transport, engine=engine, source=p)
